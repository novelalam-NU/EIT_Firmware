...
void measurement_task(void *args) {
  ...
  while (1) {
    ...
    for (uint8_t src_elec_pair = 0; src_elec_pair < NUM_ELECTRODE_PAIRS;
         src_elec_pair++) {
      for (uint8_t sense_elec_pair = 0; sense_elec_pair < NUM_SENSE_PAIRS;
           sense_elec_pair++) {
        Calibration_t *curr_config =
            &pair_calibration_map[src_elec_pair][sense_elec_pair];
        const size_t ch =
            (size_t)src_elec_pair * NUM_SENSE_PAIRS + sense_elec_pair;
        ...
        float smoothed = (1.0f - AMP_FILTER_ALPHA) * prev +
